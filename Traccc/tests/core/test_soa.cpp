...
TEST(SoA, TiledColumnsAndPadding) {
  using layout = tiled_soa_layout<32, float[3], std::uint64_t>;
  using buffer = soa_buffer<layout>;
  static_assert(!std::is_convertible_v<soa_view<layout>, test_view>);
  vecmem::host_memory_resource mr;
  buffer b(65u, mr);
  ASSERT_EQ(b.capacity(), 96u);
  soa_device<layout, false> data(b);
  for (unsigned int row = 0; row < b.size(); ++row) {
    for (unsigned int col = 0; col < 3; ++col) {
      data.at<0>(row, col) = static_cast<float>(row * 3 + col);
    }
    data.at<1>(row) = row + 1000u;
  }
  const soa_device<layout, false> read(b);
  for (unsigned int row = 0; row < b.size(); ++row) {
    for (unsigned int col = 0; col < 3; ++col) {
      EXPECT_EQ(read.at<0>(row, col), static_cast<float>(row * 3 + col));
      const auto* expected = reinterpret_cast<const float*>(
          static_cast<const char*>(b.ptr()) +
          (row / 32) * 32 * layout::total_size + col * 32 * sizeof(float));
      EXPECT_EQ(expected[row % 32], static_cast<float>(row * 3 + col));
    }
    EXPECT_EQ(read.at<1>(row), row + 1000u);
  }
}
