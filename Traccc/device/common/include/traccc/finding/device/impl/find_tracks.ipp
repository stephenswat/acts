    ...
    if (local_num_params > 0) {
      ...
      for (unsigned int i = 0; i < local_num_params; ++i) {
        ...
        if (payload.step == 0 && !sf.has_material()) {
          ...
          /*
           * The surface link is the only part of the track state that
           * `edm::make_track_state` used to contribute here; the updater
           * itself writes the vector and the covariance.
           */
