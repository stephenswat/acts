...
struct soa_layout {
  ...
  static constexpr unsigned int tile_size = 0;
...
/// SoA columns grouped into fixed-size tiles. The tile size is part of the
/// layout type, so views cannot accidentally be read using another layout.
template <unsigned int TileSize, typename... Ts>
struct tiled_soa_layout : soa_layout<Ts...> {
  using base_type = soa_layout<Ts...>;
  using size_type = typename base_type::size_type;
  static_assert(TileSize > 0);
  static_assert(((TileSize * sizeof(std::remove_all_extents_t<Ts>) %
                      base_type::max_alignment ==
                  0) &&
                 ...));
  static constexpr unsigned int tile_size = TileSize;
  static constexpr size_type padded_size(size_type n) {
    return n % TileSize == 0 ? n : n + (TileSize - n % TileSize);
  }
};
...
template <unsigned int TileSize, typename... Ts>
struct is_soa_layout_helper<tiled_soa_layout<TileSize, Ts...>> {
  static constexpr bool value = true;
};
...
template <concepts::soa_layout layout_t, bool is_in_global_memory = true,
          bool is_const = false>
class soa_device {
  static constexpr unsigned int tile_size = layout_t::tile_size;
 ...
 public:
  ...
  TRACCC_HOST_DEVICE type_at<I>& at(size_type i) {
    ...
    value_t* TRACCC_RESTRICT ptr = column<value_t>(
        static_cast<size_type>(layout_type::template offset_at<I>), i);
    if constexpr (tile_size == 0) {
      return ptr[i];
    } else {
      return ptr[i % tile_size];
    }
  }
 ...
 private:
  ...
  TRACCC_HOST_DEVICE value_t* column(size_type byte_offset,
                                     size_type row) const {
    ...
    value_t* ptr;
    if constexpr (tile_size == 0) {
      ptr = reinterpret_cast<value_t*>(base + byte_offset * m_capacity);
    } else {
      ptr = reinterpret_cast<value_t*>(
          base + (row / tile_size) * tile_size * layout_type::total_size +
